...
map_get :: proc(v: any, map_key: string) -> (dug: any, err: Template_Error) {
    ...
    // Use type_info_base to ensure we get the underlying data structxure
    ...
    for bucket_index in 0 ..< map_cap {
        ...
        key_ptr := rawptr(
            runtime.map_cell_index_dynamic(ks, map_info.ks, bucket_index),
        )
...
lexer_make :: proc() -> ^Lexer {
    ...
    l.stmds = make([dynamic]Stmd, 0, context.temp_allocator)
    l.stmd_pairs = make([dynamic]StmdPair, 0, context.temp_allocator)
    arena_init(&l.arena)
...
lexer_parse :: proc(l: ^Lexer) -> (err: Lexer_Error) {
    for l.cursor < len(l.src) {
        ...
        case lexer_peek(l, l.delim.otag_stmd):
            lexer_append(l)
            lexer_start(l, .Stmd)

        // Be careful with checking for "{{" -- it could be a substring of "{{{"
        case lexer_peek(l, l.delim.otag) &&
             l.cur_token_type != .Tag_Literal_Triple:
            lexer_append(l)
            lexer_start(l, .Tag)
...
parse_for :: proc(l: ^Lexer, t: Token)
parse_if :: proc(l: ^Lexer, t: Token)
...
template_make :: proc(l: ^Lexer) -> ^Template {
    ...
    t.lexer = l
    t.context_stack = make(
        [dynamic]Context_Stack_Entry,
        0,
...
template_string_from_key :: proc(tmpl: ^Template, key: string) -> (s: string) {
    ...
    if key == "." {
        ...
        if len(ids[1:]) > 0 {
            ...
            if is_map(resolved) ||
               is_struct(resolved) && has_key(resolved, last) {
...
template_inject_list_into_context_stack :: proc(
    tmpl: ^Template,
    list: any,
    offset: int,
) {
    ...
    end_chunk := template_find_section_close_tag_index(
        tmpl,
        section_name,
        offset,
    )
    ...
    if tmpl.layout != "" {
        ...
        for t, i in layout_lexer.tokens {
            if token_is_tag(t) && t.value == "content" {
                template_insert_content_into_layout(
                    layout_template,
                    t,
                    i,
                    rendered,
...
render_in_layout_file :: proc(
    ...
    // Read layout file.
    layout, _ := os.read_entire_file_from_filename(
        layout_filename,
        context.temp_allocator,
    )
...
render_from_filename :: proc(
    ...
    // Read template file.
    src, _ := os.read_entire_file_from_filename(
        filename,
        context.temp_allocator,
    )
...
render_with_json :: proc(
    ...
    // Load JSON.
    json_src, _ := os.read_entire_file_from_filename(
        json_filename,
        context.temp_allocator,
    )
