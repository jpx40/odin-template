...
handle_stmd:: proc(l: ^Lexer) -> (err: Lexer_Error) {
    ...
    token_text, _ = strings.remove_all(tmp_str, " ", allocator = context.temp_allocator)
    if token_text[2:4] == "for"{
        
    } else if token_text[2:3] == "if" {
    
    } else if token_text[2:5] == "else" {
        
    } else if token_text[2:6] == "endif" {
        
    } else if token_text[2:8] == "endfor" {
...
extract_for :: proc(l: ^Lexer, tmp_str: string) {
   ...
   var :=   var_i[0]
   split_in := strings.split(tmp_str, "in", allocator = context.temp_allocator)
   arr := split_inp[1]
...
split := strings.split(tmp_str, "<=", allocator = context.temp_allocator)

one  := strings.remove_all(tmp_str, "{%", allocator = context.temp_allocator)
two := strings.remove_all(tmp_str, "%}", allocator = context.temp_allocator)
...
template_insert_partial :: proc(tmpl: ^Template, token: Token, offset: int) -> (err: Lexer_Error) {
...
_main :: proc(
	...
	if layout_filename != "" {
		...
		output = render_from_filename_with_json(template_filename, json_filename) or_return
	}
